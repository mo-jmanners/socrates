...
INTEGER, PARAMETER :: n_ri_gases = 6
...
TYPE(StrRefractParam) :: RefractParam(n_ri_gases) = (/ &
  StrRefractParam(ip_n2, &            ! Gas type
                  0.4679E-06_RealK, & ! Minimum wavelength
                  2.0587E-06_RealK, & ! Maximum wavelength
                  101325.0_RealK, &   ! Pressure
                  273.15_RealK), &    ! Temperature

  StrRefractParam(ip_o2, &            ! Gas type
                  0.4000E-06_RealK, & ! Minimum wavelength
                  1.8000E-06_RealK, & ! Maximum wavelength
                  101325.0_RealK, &   ! Pressure
                  293.15_RealK), &    ! Temperature

  StrRefractParam(ip_h2o, &           ! Gas type
                  0.3500E-06_RealK, & ! Minimum wavelength
                  1.2000E-06_RealK, & ! Maximum wavelength
                  1333.0_RealK, &     ! Pressure
                  293.15_RealK), &    ! Temperature

  StrRefractParam(ip_ar, &            ! Gas type
                  0.1404E-06_RealK, & ! Minimum wavelength
                  0.5677E-06_RealK, & ! Maximum wavelength
                  101325.0_RealK, &   ! Pressure
                  273.15_RealK), &    ! Temperature

  StrRefractParam(ip_co2, &           ! Gas type
                  0.1807E-06_RealK, & ! Minimum wavelength
                  1.6945E-06_RealK, & ! Maximum wavelength
                  101325.0_RealK, &   ! Pressure
                  273.15_RealK), &    ! Temperature

  StrRefractParam(ip_ch4, &           ! Gas type
                  0.5290E-06_RealK, & ! Minimum wavelength
                  0.6585E-06_RealK, & ! Maximum wavelength
                  101325.0_RealK, &   ! Pressure
                  293.15_RealK) &     ! Temperature
  /)
...
!   E. R. Peck and B. N. Khanna. J. Opt. Soc. Am. 56, 1059-1063 (1963)
    refract_re_m1_gas = 6.8552E-05_RealK + &
        3.243157E-02_RealK/(144.0_RealK - wl_um**(-2.0_RealK))
...
!   P. Křen. Appl. Opt. 50, 6484-6485 (2011)
    refract_re_m1_gas = 1.181494E-04_RealK + &
        9.708931E-03_RealK/(75.4_RealK - wl_um**(-2.0_RealK))

  CASE (ip_h2o)
!   P. E. Ciddor. Appl. Opt. 35, 1566-1573 (1996)
    refract_re_m1_gas = 1.022E-08_RealK*(295.235_RealK + &
        2.6422_RealK*wl_um**(-2.0_RealK) - &
        0.032380_RealK*wl_um**(-4.0_RealK) + &
        0.004028_RealK*wl_um**(-6.0_RealK))

  CASE (ip_ar)
!   A. Bideau-Mehu, Y. Guern, R. Abjean, A. Johannin-Gilles. J. Quant.
!   Spectrosc. Rad. Transfer 25, 395-402 (1981)
    refract_re_m1_gas = &
        2.50141E-03_RealK/(91.012_RealK - wl_um**(-2.0_RealK)) + &
        5.00283E-04_RealK/(87.892_RealK - wl_um**(-2.0_RealK)) + &
        5.22343E-02_RealK/(214.02_RealK - wl_um**(-2.0_RealK))

  CASE (ip_co2)
!   A. Bideau-Mehu, Y. Guern, R. Abjean and A. Johannin-Gilles. Opt. Commun. 9,
!   432-434 (1973)
    refract_re_m1_gas = &
        6.99100E-02_RealK/(166.175_RealK - wl_um**(-2.0_RealK)) + &
        1.44720E-03_RealK/(79.609_RealK - wl_um**(-2.0_RealK)) + &
        6.42941E-05_RealK/(56.3064_RealK - wl_um**(-2.0_RealK)) + &
        5.21306E-05_RealK/(46.0196_RealK - wl_um**(-2.0_RealK)) + &
        1.46847E-06_RealK/(0.0584738_RealK - wl_um**(-2.0_RealK))
