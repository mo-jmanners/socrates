...
!   Number of points required
  REAL  (RealK), Intent(OUT), Dimension(n) :: root
!   Roots of the Legendre polynomial (quadrature points)
  REAL  (RealK), Intent(OUT), Dimension(n) :: weight
...
!   Number of iterations in fiding root
  INTEGER :: i
!   Loop variable
  REAL  (RealK), Dimension(n+1) :: bracket
...
!   Temporary variable used in swapping
  REAL  (RealK) :: shift
!   Absolute shift in the position of the root in this 
!   iteration
  REAL  (RealK) :: wt
!   Temporary weight
...
!
      ...
      IF (yl*yh > 0.0_RealK) THEN
        WRITE(iu_err, '(/a)') 'Error: root not bracketted.'
        stop
      ENDIF
!
...
!
  IF (ABS(sum_weight-2.0_RealK) > 1.0e+04_RealK*EPSILON(sum_weight)) THEN
    WRITE(iu_err, '(/a)')  &
      '*** Error: gaussian weights do not sum correctly.'
    ierr = i_err_fatal
    RETURN
  ENDIF
!
