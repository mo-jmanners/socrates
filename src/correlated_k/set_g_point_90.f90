...
!   Pointer for clarity of coding
  INTEGER :: ntop
!   Split top k-term into ntop+1 parts
!
...
!   Product of the absorption coefficient and the weighting function
  REAL  (RealK) :: kl
!   Absorption coefficient at the left of the chopping interval
  REAL  (RealK) :: kr
!   Absorption coefficient at the left of the chopping interval
  REAL  (RealK) :: km
...
! error suhould be proprtional to the inverse second power of the
! number of terms.
...
!   Split top k-term into ntop+1 parts:
    ntop=0
!    IF (n_k > 5) ntop=(n_k-5)/3
!    IF (n_k > 2) ntop=1

    dlogk_inc=dlogk/REAL(n_k-ntop, RealK)
...
!     with absorption greater than ktop fall in the next interval.
      ktop(i)=kabs(n_nu)/EXP(REAL(n_k-ntop-i, RealK)*dlogk_inc)
      IF (i >= n_k-ntop) THEN
        ktop(i)=kabs(n_nu)/EXP(REAL(n_k-i, RealK)*dlogk_inc/(ntop+1))
      END IF
!
...
!     Set the left and right-hand ends of the interval
      ...
      DO
        ...
        IF (km > ktop(i)) THEN
          ir=im
          kr=km
        ELSE IF (km <= ktop(i)) THEN
          il=im
          kl=km
        ENDIF
      ENDDO
!     Ensure that at least one k-interval is available for each term.
      ig(i)=MIN(il, n_nu-n_k+i)
      i0 = ig(i-1)+1
      i1 = ig(i)
      n_nu_k=i1-i0+1
