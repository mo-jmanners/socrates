...
integer :: ios, i, j, k, iu_qtxt, iu_parsum, temp
real(RealK) :: qtemp
character(len=256) :: filename
character(len=256) :: parsum_file='parsum_2025.dat'
...
do i = 1, number_species
  ...
  if (l_exist) then
    open(newunit=iu_qtxt, FILE=trim(filename), iostat=ios, status='old')
    do
      read(iu_qtxt, '(i4,f30.5)', iostat=ios) temp, qtemp
      if (ios /= 0) exit
      qcoeff(temp:, i) = qtemp
    end do
    close(iu_qtxt)
...
open(newunit=iu_parsum, FILE=trim(parsum_file), iostat=ios, status='new')
