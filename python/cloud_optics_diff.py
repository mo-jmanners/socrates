...
if __name__ == '__main__':
    if (len(sys.argv) > 5):
        ...
        raise RuntimeError('please enter two fit filenames to compare')

extinction2=np.zeros(4096*3)
ss_albedo2=np.zeros(4096*3)
asymmetry2=np.zeros(4096*3)
            ...
            if (band >= start_band):
                ...
                if (scheme == 20 or scheme == 5):
                    for n in range(0, 4096):
                        ...
                        extinction2[(band-start_band)*4096 + n] = extinction[n]
                        ss_albedo2[(band-start_band)*4096 + n] = ss_albedo[n]
                        asymmetry2[(band-start_band)*4096 + n] = asymmetry[n]
            ...
            if (band >= start_band):
                ...
                if (scheme == 20 or scheme == 5):
                    ...
                    ax[plotrow,0].plot(re*1e6, extinction,
                        ...
                        label='Extinction (m2/kg): '+label2)
                    ax[plotrow,0].plot(re*1e6, scattering,
                        linestyle='dotted', linewidth=1.0, color='green',
                        label='Scattering (m2/kg): '+label2)
                    ax[plotrow,1].plot(re*1e6, ss_albedo,
                        linestyle='dotted', linewidth=1.0, color='green',
                        label='Single Scattering Albedo: '+label2)
                    ax[plotrow,2].plot(re*1e6, asymmetry,
                        linestyle='dotted', linewidth=1.0, color='blue',
                        label='Asymmetry: '+label2)
                    for n in range(0, 4096):
                        extinction[n] = extinction2[(band-start_band)*4096 + n]
                        ss_albedo[n] = ss_albedo2[(band-start_band)*4096 + n]
                        scattering[n] = extinction[n] * ss_albedo[n]
                        asymmetry[n] = asymmetry2[(band-start_band)*4096 + n]
                    ax[plotrow,0].plot(re*1e6, extinction,
                        linestyle='solid', linewidth=1.0, color='black',
                        label='Extinction (m2/kg): '+label1)
                    ax[plotrow,0].plot(re*1e6, scattering,
                        linestyle='solid', linewidth=1.0, color='green',
                        label='Scattering (m2/kg): '+label1)
                    ax[plotrow,1].plot(re*1e6, ss_albedo,
                        linestyle='solid', linewidth=1.0, color='green',
                        label='Single Scattering Albedo: '+label1)
                    ax[plotrow,2].plot(re*1e6, asymmetry,
                        linestyle='solid', linewidth=1.0, color='blue',
                        label='Asymmetry: '+label1)
